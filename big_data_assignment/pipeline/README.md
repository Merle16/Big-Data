...
**Features (46 total):** base numerics (`startYear`, `numVotes_log1p`, `runtimeMinutes`), title lexical features, crew counts, director/writer avg birth years, LOO-smoothed hit-rate encodings (Bayesian smoothing C=5), TF-IDF cosine similarity to hit/non-hit centroids, PageRank (4 cols with era-stratified decade fallback), and 23 binary genre flags.

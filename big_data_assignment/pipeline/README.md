...
## Final submission — temporal holdout evaluation (`pipeline/temporal_holdout_eval.py`)

The competition submission is generated by a self-contained script that does **not** import from the pipeline package. It implements a stricter evaluation protocol than the internal 80/20 random split used during development.

**Why a separate script and not `run.py`?**
The internal pipeline uses a random 80/20 split for validation. This inflates AUC because directors and writers present in the internal validation set also appear in the training set (same-pool OOF effect). The temporal split used here — train on films released ≤ 2013, evaluate on films from 2014 onward — is a genuine out-of-distribution test whose AUC tracks leaderboard performance much more faithfully.

**Features (43 total):** base numerics (`startYear`, `numVotes_log1p`, `runtimeMinutes`), title lexical features, crew counts, director/writer avg birth years, LOO-smoothed hit-rate encodings (Bayesian smoothing C=5), PageRank (4 cols with era-stratified decade fallback), and 23 binary genre flags. TF-IDF cosine similarity features (`title_sim_to_hit`, `title_sim_to_non_hit`, `title_sim_margin`) were removed after ablation showed they hurt temporal holdout AUC by −0.020.

**Threshold calibration:** Youden's J (argmax TPR − FPR) is computed on the temporal holdout. Because training hit rate (~53%) exceeds the hidden validation hit rate (~41%), the optimal threshold is well below 0.5. Using the inflated internal threshold (0.65) caused leaderboard score of 0.6921; switching to temporal Youden threshold brought it to 0.8503.

**CLI flags:**

```bash
# Standard run — era-stratified PageRank fallback, Youden's J threshold
python pipeline/temporal_holdout_eval.py

# + random hyperparameter search (40 trials, live leaderboard printed to terminal)
python pipeline/temporal_holdout_eval.py --tune --n-trials 40

# Force a fixed submission threshold — ladder-test without re-running search
python pipeline/temporal_holdout_eval.py --threshold 0.50

# Ablation: global-median PageRank fallback (control condition for era-stratified experiment)
python pipeline/temporal_holdout_eval.py --global-fallback
```

**Prerequisites:** PageRank parquets must exist (`pipeline/outputs/graph/features_pagerank*.parquet`). Run the full pipeline first:
...
| Submission | Val True count | Score |
|---|---|---|
| Pipeline default (random split, t=0.65) | 312 / 955 | 0.6921 |
| + Genre + PageRank features (t=0.45) | 477 / 955 | 0.7958 |

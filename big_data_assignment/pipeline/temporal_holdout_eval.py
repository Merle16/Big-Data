...
# ── Paths ──────────────────────────────────────────────────────────────────────
ROOT = Path(__file__).resolve().parent.parent   # big_data_assignment/
...
TITLE_PRINCIPALS = ROOT / "data" / "raw" / "IMDB_external_csv" / "title_principals.csv"
...
FEATURE_COLS = [
    ...
    # actor hit-rate encoding (lead actor, top-3 average)
    "lead_actor_hit_rate", "top3_actor_hit_rate",
    # pagerank
...
def load_actor_index(path: Path) -> Tuple[Dict[str, List[str]], Dict[str, List[str]]]:
    """
    Return (lead_idx, top3_idx) from title_principals.csv.

    lead_idx  : {tconst: [nconst]}         — ordering=1 actor/actress only
    top3_idx  : {tconst: [nconst, ...]}    — top-3 billed actors/actresses

    Using only actor/actress categories (not directors/writers, which are
    already captured by director_hit_rate and writer_hit_rate).
    """
    _BAD = {"", r"\N", "\\N"}
    ACTOR_CATS = {"actor", "actress"}
    print("  Loading title_principals.csv …")
    tp = pd.read_csv(
        path,
        usecols=["tconst", "ordering", "nconst", "category"],
        dtype=str,
    )
    tp = tp[tp["category"].isin(ACTOR_CATS)].copy()
    tp["ordering"] = pd.to_numeric(tp["ordering"], errors="coerce")
    tp = tp.dropna(subset=["ordering"]).sort_values(["tconst", "ordering"])

    lead_idx: Dict[str, List[str]]  = defaultdict(list)
    top3_idx: Dict[str, List[str]]  = defaultdict(list)
    for tc, grp in tp.groupby("tconst"):
        tc = str(tc).strip()
        nconsts = [str(n).strip() for n in grp["nconst"] if str(n).strip() not in _BAD]
        if nconsts:
            lead_idx[tc] = [nconsts[0]]
            top3_idx[tc] = nconsts[:3]
    return lead_idx, top3_idx
...
def build_feature_matrix(
    ...
    lead_actor_hit_rates: np.ndarray,
    top3_actor_hit_rates: np.ndarray,
    ...
    # OOF hit rates (crew + actors)
    base["director_hit_rate"]   = dir_hit_rates
    base["writer_hit_rate"]     = wri_hit_rates
    base["lead_actor_hit_rate"] = lead_actor_hit_rates
    base["top3_actor_hit_rate"] = top3_actor_hit_rates
...
_PARAM_GRID = {
    ...
    # Include the default (10) which was absent before — heavily regularized
    # models generalise better to the hidden val
    "min_child_weight": [3, 5, 7, 10, 15],
    "gamma":            [0.0, 0.1, 0.2, 0.5],
    # Include the default (0.5) which was absent before
    "reg_alpha":        [0.0, 0.1, 0.3, 0.5, 0.7],
...
def main():
    ...
    lead_actor_idx, top3_actor_idx = load_actor_index(TITLE_PRINCIPALS)
    print(f"  crew index: {len(dir_idx)} tconsts with directors, "
          f"{len(wri_idx)} tconsts with writers")
    print(f"  actor index: {len(lead_actor_idx)} tconsts with lead actor, "
          f"{len(top3_actor_idx)} tconsts with top-3 cast")
    ...
    # ── TRAIN features ──────────────────────────────────────────────────────
    print("\n[5] Engineering TRAIN features (LOO encoding) …")

    global_mean_dir   = float(train_df["label"].mean())
    global_mean_wri   = global_mean_dir  # same base rate
    global_mean_actor = global_mean_dir  # same base rate
    ...
    lead_actor_loo = compute_loo_hit_rate(train_df, lead_actor_idx,  global_mean_actor)
    top3_actor_loo = compute_loo_hit_rate(train_df, top3_actor_idx,  global_mean_actor)
    print(f"  director LOO: mean={dir_loo.mean():.4f}, std={dir_loo.std():.4f}")
    print(f"  writer   LOO: mean={wri_loo.mean():.4f}, std={wri_loo.std():.4f}")
    print(f"  lead-act LOO: mean={lead_actor_loo.mean():.4f}, std={lead_actor_loo.std():.4f}")
    print(f"  top3-act LOO: mean={top3_actor_loo.mean():.4f}, std={top3_actor_loo.std():.4f}")
    ...
    train_feat = build_feature_matrix(
        train_df, dir_loo, wri_loo, lead_actor_loo, top3_actor_loo,
        train_genre, train_crew, train_birth
    )
    ...
    lead_actor_lookup = build_entity_lookup(train_df, lead_actor_idx, global_mean_actor)
    top3_actor_lookup = build_entity_lookup(train_df, top3_actor_idx, global_mean_actor)
    ...
    lead_actor_hld = apply_entity_lookup(holdout_df, lead_actor_idx, lead_actor_lookup, global_mean_actor)
    top3_actor_hld = apply_entity_lookup(holdout_df, top3_actor_idx, top3_actor_lookup, global_mean_actor)
    ...
    hld_feat = build_feature_matrix(
        holdout_df, dir_hld, wri_hld, lead_actor_hld, top3_actor_hld,
        hld_genre, hld_crew, hld_birth
    )
    ...
    # OOF director/writer/actor hit rates on full labeled set via 5-fold CV
    ...
    all_lead_actor_oof = np.full(len(all_labeled), all_gm)
    all_top3_actor_oof = np.full(len(all_labeled), all_gm)
    ...
    for fold_idx, (fold_tr, fold_val) in enumerate(
        ...
        fold_gm       = float(fold_train_df["label"].mean())
        d_lkp  = build_entity_lookup(fold_train_df, dir_idx,        fold_gm)
        w_lkp  = build_entity_lookup(fold_train_df, wri_idx,        fold_gm)
        la_lkp = build_entity_lookup(fold_train_df, lead_actor_idx, fold_gm)
        t3_lkp = build_entity_lookup(fold_train_df, top3_actor_idx, fold_gm)
        all_dir_oof[fold_val]        = apply_entity_lookup(fold_val_df, dir_idx,        d_lkp,  fold_gm)
        all_wri_oof[fold_val]        = apply_entity_lookup(fold_val_df, wri_idx,        w_lkp,  fold_gm)
        all_lead_actor_oof[fold_val] = apply_entity_lookup(fold_val_df, lead_actor_idx, la_lkp, fold_gm)
        all_top3_actor_oof[fold_val] = apply_entity_lookup(fold_val_df, top3_actor_idx, t3_lkp, fold_gm)
        print(f"    Fold {fold_idx + 1}/5 done.")

    # Build full-data lookups for hidden inference
    ...
    lead_actor_lookup_full = build_entity_lookup(all_labeled, lead_actor_idx, all_gm)
    top3_actor_lookup_full = build_entity_lookup(all_labeled, top3_actor_idx, all_gm)
    ...
    all_feat = build_feature_matrix(
        ...
        all_lead_actor_oof, all_top3_actor_oof,
        all_genre, all_crew, all_birth)
    ...
    def build_hidden_features(clean_path: Path, label_name: str) -> pd.DataFrame:
        ...
        lead_actor_rates = apply_entity_lookup(hid_df, lead_actor_idx, lead_actor_lookup_full, all_gm)
        top3_actor_rates = apply_entity_lookup(hid_df, top3_actor_idx, top3_actor_lookup_full, all_gm)
        ...
        feat = build_feature_matrix(
            hid_df, dir_rates, wri_rates, lead_actor_rates, top3_actor_rates,
            genre_f, crew_f, birth_f)
